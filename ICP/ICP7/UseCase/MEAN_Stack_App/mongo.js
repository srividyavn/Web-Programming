...
var url='mongodb+srv://dbuser:dbpwd@cluster0-5tqz8.mongodb.net/librarysystem';
...
app.get('/get', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        dbase.collection('library').find().toArray(function(err, result){
            if(err)
            {
...
app.get('/delete/:toBeDeleted_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        dbase.collection('library').deleteOne({_id: ObjectID(req.params.toBeDeleted_id)}, function(err, result){
            if(err)
            {
...
app.post('/update/:toBeUpdated_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        dbase.collection("library").updateMany(myquery, newvalues, function (err, res) {
            if (err) throw err;
...
var insertDocument = function(db, data, callback) {
    db.collection('library').insertOne( data, function(err, result) {
        if(err)
        {
